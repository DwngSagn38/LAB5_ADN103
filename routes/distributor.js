...
// Tìm các nhà phân phối có tên phù hợp với 'key' được cung cấp bằng cách sử dụng regex không phân biệt chữ hoa/thường
...
router.delete('/destroy-distributor-by-id/:id',async(req,res)=>{
    try {
        const {id} = req.params;
        const result = await Distributors.deleteOne({_id : id});
        if(result.length>0){
            res.json({
                status:200,
                mess:"Xoá thành công",
